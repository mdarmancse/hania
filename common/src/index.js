...
const FirebaseProvider  = ({ config, appcat, children }) => {
    ...
    if (!app.apps.length) {
        ...
        firebase = {
            ...
            driverDocsRef1:(uid,timestamp) => app.storage().ref(`users/${uid}/driver1/${timestamp}/`),
            driverDocsRef2:(uid,timestamp) => app.storage().ref(`users/${uid}/driver2/${timestamp}/`),
            driverDocsRef3:(uid,timestamp) => app.storage().ref(`users/${uid}/driver3/${timestamp}/`),
            driverDocsRef4:(uid,timestamp) => app.storage().ref(`users/${uid}/driver4/${timestamp}/`),           
            ...
            bookingLaterRef:(uid,role) =>
                role == 'rider'? app.database().ref('bookings').orderByChild('customer').equalTo(uid).limitToFirst(6):
                    (role == 'driver'?
                            app.database().ref('bookings').orderByChild('driver').equalTo(uid).limitToFirst(6)
                            :
                            (role == 'fleetadmin'?
                                    app.database().ref('bookings').orderByChild('fleetadmin').equalTo(uid).limitToFirst(6)
                                    : app.database().ref('bookings').limitToFirst(6)
                            )
                    ),
