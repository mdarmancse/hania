...
export const mainSignUp = (regData) => async (firebase) => {
  ...
  await driverDocsRef1(timestamp).put(regData.img1);
  regData.img1 = await driverDocsRef1(timestamp).getDownloadURL();
  await driverDocsRef2(timestamp).put(regData.img2);
  regData.img2 = await driverDocsRef2(timestamp).getDownloadURL();
  await driverDocsRef3(timestamp).put(regData.img3);
  regData.img3 = await driverDocsRef3(timestamp).getDownloadURL();
  await driverDocsRef4(timestamp).put(regData.img4);
  regData.img4 = await driverDocsRef4(timestamp).getDownloadURL();
...
export const updateProfile = (userAuthData, updateData) => (dispatch) => async (firebase) => {
  ...
  if (updateData.img1) {
    ...
    await driverDocsRef1(timestamp).put(updateData.img1);
    updateData.img1 = await driverDocsRef1(timestamp).getDownloadURL();
  }

  if (updateData.img2) {
    let timestamp = new Date().toISOString();
    await driverDocsRef2(timestamp).put(updateData.img2);
    updateData.img2 = await driverDocsRef2(timestamp).getDownloadURL();
  }

  if (updateData.img3) {
    let timestamp = new Date().toISOString();
    await driverDocsRef3(timestamp).put(updateData.img3);
    updateData.img3 = await driverDocsRef3(timestamp).getDownloadURL();
  }

  if (updateData.img4) {
    let timestamp = new Date().toISOString();
    await driverDocsRef4(timestamp).put(updateData.img4);
    updateData.img4 = await driverDocsRef4(timestamp).getDownloadURL();
  }
