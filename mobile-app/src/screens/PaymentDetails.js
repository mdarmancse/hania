...
export default function PaymentDetails(props) {
  ...
  const [payDetails, setPayDetails] = useState({
    ...
    payableAmount1: booking.trip_cost - booking.discount - booking.usedWalletMoney,
  ...
  const promoHeaderCom = { icon: 'ios-close', type: 'ionicon', color: colors.BLACK, size: 35, component: TouchableWithoutFeedback, onPress: () => { setPromodalVisible(false) } };
  ...
  const rCom= userdata && userdata.usertype == 'rider' && booking.status =='PAYMENT_PENDING'?<TouchableOpacity onPress={cancelCurBooking}><Text style={{color:colors.BLACK, marginTop: 5,}}>{t('cancel')}</Text></TouchableOpacity>:null;
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollStyle}>
        <View style={{ flex: 1, flexDirection: 'column' }}>
          <View style={{ flex: 1, flexDirection:isRTL?'row-reverse':'row', justifyContent: 'space-between', paddingLeft: 20, paddingRight: 20, marginBottom: 4 }}>
            <Text style={{ color: colors.BLACK, textAlign:isRTL?'right':'left', lineHeight: 45, fontSize: 16, fontWeight: '500' }}>{t('bill_details')}</Text>
            {userdata && userdata.usertype == 'rider' && (booking.status == 'PAYMENT_PENDING' || (appcat == 'taxi' && booking.status == "PENDING"))? 
                ...
                <TouchableOpacity
                  ...
                  <Text style={{ color: colors.START_TRIP, textAlign:isRTL?'right':'left', lineHeight: 45, fontSize: 12, fontWeight: '500' }}>{t('apply_promo')}</Text>
                </TouchableOpacity>
              : null}
          </View>
          ...
          {userdata ?
            ...
            : null}
         {userdata && booking.status =='PAYMENT_PENDING' ?
            <View style={{ flex: 1, flexDirection: isRTL?'row-reverse':'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              <Text style={{ color: colors.BLACK, textAlign:isRTL?'right':'left', lineHeight: 45, fontSize: 18, fontWeight: '500' }}>{t('payable_ammount')}</Text>
              {booking.status =='PAYMENT_PENDING'?
                <Text style={{ color: colors.NEW, textAlign:isRTL?'right':'left', lineHeight: 45, fontSize: 18, fontWeight: 'bold' }}>{settings.symbol} {payDetails.payableAmount ? parseFloat(payDetails.payableAmount).toFixed(settings.decimal) : 0.00}</Text>
                :
                null }
            </View>
            : null}

            {userdata && booking.status =='REACHED' ?
            <View style={{ flex: 1, flexDirection: isRTL?'row-reverse':'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              {booking.status =='REACHED' ?
              <Text style={{ color: colors.BLACK, textAlign:isRTL?'right':'left', lineHeight: 45, fontSize: 18, fontWeight: '500' }}>{t('payable_ammount')}</Text>
              : null}
              {booking.status =='REACHED' && payDetails.payableAmount1 < 0 ?
                <Text style={{ color: colors.BLACK, textAlign:isRTL?'right':'left', lineHeight: 45, fontSize: 18, fontWeight: 'bold' }}>{t('full_paid')}</Text>
                :
                <Text style={{ color: colors.BLACK, textAlign:isRTL?'right':'left', lineHeight: 45, fontSize: 18, fontWeight: 'bold' }}>{settings.symbol} {payDetails.payableAmount1 ? parseFloat(payDetails.payableAmount1).toFixed(settings.decimal) : 0.00}</Text> }
            </View>
            : null}
        </View>
