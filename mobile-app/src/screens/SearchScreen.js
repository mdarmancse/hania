...
export default function SearchScreen(props) {
  ...
  const searchLocation = async (text) => {
    ...
    if(text.length>(settings.AllowCriticalEditsAdmin?0:0)){
  ...
  const lCom ={ icon: 'angle-left', type: 'font-awesome', color: colors.BLACK, size: 35, component: TouchableWithoutFeedback, onPress: () => { props.navigation.goBack(); } };
  const rCom = { icon: 'angle-right', type: 'font-awesome', color: colors.BLACK, size: 35, component: TouchableWithoutFeedback, onPress: () => { props.navigation.goBack(); } };
  const okCom = <TouchableOpacity onPress={okClicked}><Text style={[styles.headerTitleStyle,{color:colors.BLACK}]}>{t('ok')}</Text></TouchableOpacity>
  return (
              ...
              renderItem={({item, index}) => {
                return (
                  <View key={"key" + index} style={{flexDirection: 'row', width:'100%',marginVertical:10}}>
                    <Text style={{paddingLeft:10, width: width - 40, fontSize:15, color:colors.BLACK, paddingTop: 6, paddingBottom:6}} numberOfLines={1} >{item.add}</Text> 
                    <TouchableOpacity
                      style={{paddingLeft:0}}
