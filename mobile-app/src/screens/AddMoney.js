...
export default function AddMoneyScreen(props) {
  ...
  const newData = ({ item, index }) => {
    return (
      <TouchableOpacity style={[styles.boxView, { backgroundColor: item.selected ? colors.NEW : colors.BACKGROUND_PRIMARY }]} onPress={() => { quckAdd(index); }}>
        {settings.swipe_symbol===false?
          <Text style={[styles.quckMoneyText, { color: item.selected ? colors.BLACK : colors.BLACK }]} >{settings.symbol}{item.amount}</Text>
          :
          <Text style={[styles.quckMoneyText, { color: item.selected ? colors.BLACK : colors.BLACK }]} >{item.amount}{settings.symbol}</Text>
        }
      </TouchableOpacity>
    )
  }
  ...
  const lCom = { icon:isRTL ? 'ios-arrow-forward':'arrow-back-sharp', type: 'ionicon', color: colors.BLACK, size: 30, component: TouchableWithoutFeedback, onPress: () => { goBack() } }
  const rCom = { icon:isRTL ? 'ios-arrow-forward':'arrow-back-sharp', type: 'ionicon', color: colors.BLACK, size: 30, component: TouchableWithoutFeedback, onPress: () => { goBack() } }
