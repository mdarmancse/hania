...
export default function MapScreen(props) {
    ...
    const onPressBook = async () => {
        setCheckType(true);
        setBookLoading(true);
        if(!(profile.mobile && profile.mobile.length > 6) || profile.firstName == ' ' || profile.lastName == ' ' ){
...
/>
            ...
            <View style={styles.buttonBar}>
                <Button
                    ...
                    onPress={onPressBook}
                    buttonStyle={[styles.buttonStyle, { backgroundColor: appcat == 'taxi' ? colors.NEW : colors.NEW, width:bookLoading?width:width }]}
                    containerStyle={[styles.buttonContainer,{width:bookLoading?width:width}]}
                />
            </View>
            ...
            <View style={styles.fullCarView}>
                <ScrollView horizontal={true} style={styles.fullCarScroller} showsHorizontalScrollIndicator={false}>
                    {allCarTypes.map((prop, key) => {
                        return (
                            <View key={key} style={[styles.cabDivStyle, { backgroundColor: prop.active == true ? colors.NEW : colors.WHITE }]}>
                                ...
                                <View style={styles.textViewStyle}>
                                    <Text style={styles.text11}>{prop.name.toUpperCase()}</Text>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 10 }}>
                                        {isRTL?
                                            null:
                                            settings.swipe_symbol===false?
                                            <Text style={[styles.text22, { fontWeight: 'bold', color: colors.BLACK }]}>{settings.symbol}{prop.rate_per_unit_distance} / {settings.convert_to_mile ? t('mile') : t('km')} </Text>
                                            :
                                            <Text style={[styles.text22, { fontWeight: 'bold', color: colors.BLACK }]}>{prop.rate_per_unit_distance}{settings.symbol} / {settings.convert_to_mile ? t('mile') : t('km')} </Text>

                                        }
                                        {isRTL?
                                            settings.swipe_symbol===false?
                                                <Text style={[styles.text22, { fontWeight: 'bold', color: colors.MAP_TEXT }]}>{settings.symbol}{prop.rate_per_unit_distance} / {settings.convert_to_mile ? t('mile') : t('km')} </Text>
                                                :
                                                <Text style={[styles.text22, { fontWeight: 'bold', color: colors.MAP_TEXT }]}>{prop.rate_per_unit_distance}{settings.symbol} / {settings.convert_to_mile ? t('mile') : t('km')} </Text>
                                        :null}

                                    </View>
                                    <View>
                                        <Text style={styles.text22}>({prop.minTime != '' ? prop.minTime : t('not_available')})</Text>
                                    </View>
                                </View>
                            </View>
                        );
                ...
                <View style={styles.bar} ></View>
                    ...
                    <Animated.View style={{ alignItems: 'center', backgroundColor: colors.BACKGROUND_PRIMARY, flex: animation, paddingTop: 6 }}>
                        ...
                        {allCarTypes.map((prop, key) => {
                            return (
                                <TouchableOpacity 
                                style={[styles.carContainer,{ backgroundColor: prop.active == true ? colors.GRAY : colors.WHITE}]}
                            ...
                            <View style={[styles.bodyContent, {alignContent:'center',flexDirection:'column', justifyContent:'center'}]}>
                                ...
                                {settings.swipe_symbol===false?
                                    <Text style={[styles.text2, { fontWeight: 'bold', color: colors.BLACK }]}>{settings.symbol}{prop.rate_per_unit_distance} / {settings.convert_to_mile ? t('mile') : t('km')} </Text>
                                    :
                                    <Text style={[styles.text2, { fontWeight: 'bold', color: colors.BLACK }]}>{prop.rate_per_unit_distance}{settings.symbol} / {settings.convert_to_mile ? t('mile') : t('km')} </Text>
                                }
                                { prop.extra_info && prop.extra_info != '' ?
                                    <View style={{ justifyContent: 'space-around',  marginLeft: 3 }}>
                                        {
                                            prop.extra_info.split().map((ln) => <Text key={ln} style={[styles.text2, { fontWeight: 'bold', color: colors.BLACK }]} >{ln}</Text>)
                                        }
                                    </View>
                                : null }
                                </View>
