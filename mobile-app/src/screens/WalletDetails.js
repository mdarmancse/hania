...
export default function WalletDetails(props) {
  ...
  const rCom = auth.info && auth.info.profile && (auth.info.profile.usertype =='driver' || (auth.info.profile.usertype =='rider' && settings && settings.RiderWithDraw))?<TouchableOpacity onPress={doWithdraw}><Text style={{color:colors.BLACK, marginTop: 5}}>{t('withdraw')}</Text></TouchableOpacity>:null;
        ...
        <View style={{ height: walletBar, marginBottom: 12 }}>
          <View >
            <View style={{ flexDirection:isRTL?'row-reverse':'row', justifyContent: "space-around", marginTop: 8 }}>
              <View style={{ height: walletBar - 50, width: '48%', backgroundColor: colors.WHITE, borderColor: colors.BLACK, borderWidth: 1, borderRadius: 8, justifyContent: 'center', flexDirection: 'column' }}>
                <Text style={{ textAlign: 'center', fontSize: 18 }}>{t('wallet_ballance')}</Text>
                {settings.swipe_symbol===false?
                  <Text style={{ textAlign: 'center', fontSize: 25, fontWeight: '500', color: colors.BLACK }}>{settings.symbol}{auth.info && auth.info.profile ? parseFloat(auth.info.profile.walletBalance).toFixed(settings.decimal) : ''}</Text>
                  :
                  <Text style={{ textAlign: 'center', fontSize: 25, fontWeight: '500', color: colors.BLACK }}>{auth.info && auth.info.profile ? parseFloat(auth.info.profile.walletBalance).toFixed(settings.decimal) : ''}{settings.symbol}</Text>
                }
              </View>
              <TouchableWithoutFeedback onPress={doReacharge}>
                <View style={{ height: walletBar - 50, width: '48%', backgroundColor: colors.NEW , borderRadius: 8, justifyContent: 'center', flexDirection: 'column' }}>
                  <Icon
                    name='add-circle'
                    type='MaterialIcons'
                    color={colors.BLACK}
                  ...
                  />
                  <Text style={{ textAlign: 'center', fontSize: 18, color: colors.BLACK }}>{t('add_money')}</Text>
                </View>
              </TouchableWithoutFeedback>
            </View>
          </View>
