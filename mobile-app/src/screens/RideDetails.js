...
export default function RideDetails(props) {
    ...
    const lCom ={ icon: 'ios-arrow-back', type: 'ionicon', color: colors.BLACK, size: 30, component: TouchableWithoutFeedback, onPress: () => { goBack() } };
                    ...
                    <View style={[styles.userDesc, {flexDirection:isRTL?'row-reverse':'row'}]}>
                        ...
                        <View style={[styles.userView, isRTL?{paddingRight:28}:{paddingLeft:28}]}>
                            {settings.swipe_symbol===false?
                                <Text style={[styles.textStyle,{textAlign:isRTL? "right":"left"}]}>{settings.symbol}{paramData && paramData.driver_share ? parseFloat(paramData.driver_share).toFixed(settings.decimal) : paramData && paramData.estimate ? paramData.estimate : 0}</Text>
                                :
                                <Text style={[styles.textStyle,{textAlign:isRTL? "right":"left"}]}>{paramData && paramData.driver_share ? parseFloat(paramData.driver_share).toFixed(settings.decimal) : paramData && paramData.estimate ? paramData.estimate : 0}{settings.symbol}</Text>
                            }
                        </View>
                    </View>
                </View>
                ...
                {paramData && ['PENDING','PAID','COMPLETE'].indexOf(paramData.status) != -1 ?
                    <View style={styles.billView}>
                        ...
                        <View style={[styles.paybleAmtView,{flexDirection:isRTL?'row-reverse':'row'}]}>
                            ...
                            {settings.swipe_symbol===false?
                                <Text style={[styles.billAmount2,{textAlign:isRTL?'right':'left'}]}>{settings.symbol}{paramData && paramData.driver_share ? parseFloat(paramData.driver_share).toFixed(settings.decimal) : null}</Text>
                                :
                                <Text style={[styles.billAmount2,{textAlign:isRTL?'right':'left'}]}>{paramData && paramData.driver_share ? parseFloat(paramData.driver_share).toFixed(settings.decimal) : null}{settings.symbol}</Text>
                            }
                        </View>
                    </View>
                    : null}
