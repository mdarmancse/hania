...
export default function SideMenu(props){
    ...
    const sideMenuList = [
        ...
        {name: t('booking_later'), navigationName: 'BookingLater', icon: 'minus', type: 'font-awesome'},
        ...
        {name: t('help_screen'), navigationName: 'Help', icon: 'ios-sad', type: 'ionicon'},
        ...
        //zinaaaaaaaaaaa je doit ajouter reservation plus tard
    ];

    const sideMenuList2 = [
        {name: t('convert_to_driver'), navigationName: 'Convert', icon: 'car-sport-outline', type: 'ionicon'}
    ];
    ...
    return (
        <View style={styles.mainViewStyle}>
            {auth.info && auth.info.profile?
                <SideMenuHeader headerStyle={styles.myHeader} userPhoto={auth.info.profile.profile_image} userEmail={auth.info.profile.mobile} userName={auth.info.profile.firstName + ' ' + auth.info.profile.lastName} language={language}></SideMenuHeader>
            :null}
            <View style={styles.compViewStyle}>
                ...
                <FlatList
                    ...
                    renderItem={({item, index}) => {
                        if (auth.info.profile.usertype == 'rider' && (item.navigationName == 'DriverTrips' || item.navigationName == 'MyEarning' || item.navigationName == 'BookingLater')) {
                            return null;
                            ...
                            return (
                                <TouchableOpacity
                                    ...
                                    style={
                                        [styles.menuItemView, {
                                            marginTop: (index == sideMenuList.length - 0) ? 30 : 0,
                                            flexDirection: isRTL ? 'row-reverse' : 'row'
                                        }]
                                    }>
                                    <View style={styles.viewIcon}>
                                    ...
                                    {item.icon == 'sign-out' ?
                                        <View style={[styles.vertialLine, {height: 20, top: -19}]}></View>
                                        :
                                        <View style={[styles.vertialLine, {height: 20, top: 24}]}></View>
                                    }
                                </TouchableOpacity>
            ...
            <View style={styles.compViewStyle1}>
                {/* <View style={[styles.vertialLine, { height: (width <= 320) ? width / 1.32 : width / 1.40, left: isRTL? 231 : 22 }]}></View> */}
                {!!settings && auth.info && auth.info.profile?
                <FlatList
                    data={sideMenuList2}
                    ...
                    renderItem={({ item, index }) => {
                        ...
                        if (auth.info.profile.usertype == 'driver'  && item.navigationName == 'Convert') {
                            return null;
                        }
                        else{
                            return(
                            <TouchableOpacity
                                ...
                                <Text style={styles.menuName1}>{item.name}</Text>
                            </TouchableOpacity>
